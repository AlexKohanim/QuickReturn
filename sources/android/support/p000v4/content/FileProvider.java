...
public class FileProvider extends ContentProvider {
    ...
    private static final File DEVICE_ROOT = new File(TiUrl.PATH_SEPARATOR);
    ...
    static class SimplePathStrategy implements PathStrategy {
        ...
        public Uri getUriForFile(File file) {
            ...
            try {
                ...
                if (rootPath2.endsWith(TiUrl.PATH_SEPARATOR)) {
                    path = path2.substring(rootPath2.length());
                } else {
                    path = path2.substring(rootPath2.length() + 1);
                }
                return new Builder().scheme("content").authority(this.mAuthority).encodedPath(Uri.encode((String) mostSpecific.getKey()) + '/' + Uri.encode(path, TiUrl.PATH_SEPARATOR)).build();
            } catch (IOException e) {
