...
class AppCompatDelegateImplV7 extends AppCompatDelegateImplBase implements Callback, LayoutInflaterFactory {
    ...
    private static final class PanelFeatureState {
        ...
        public void setStyle(Context context) {
            ...
            widgetTheme.resolveAttribute(C0191R.attr.actionBarPopupTheme, outValue, true);
            if (outValue.resourceId != 0) {
                widgetTheme.applyStyle(outValue.resourceId, true);
            }
            widgetTheme.resolveAttribute(C0191R.attr.panelMenuListTheme, outValue, true);
            if (outValue.resourceId != 0) {
                widgetTheme.applyStyle(outValue.resourceId, true);
            } else {
                widgetTheme.applyStyle(C0191R.style.Theme_AppCompat_CompactMenu, true);
            }
            Context context2 = new ContextThemeWrapper(context, 0);
            context2.getTheme().setTo(widgetTheme);
            this.listPresenterContext = context2;
            TypedArray a = context2.obtainStyledAttributes(C0191R.styleable.AppCompatTheme);
            this.background = a.getResourceId(C0191R.styleable.AppCompatTheme_panelBackground, 0);
            this.windowAnimations = a.getResourceId(C0191R.styleable.AppCompatTheme_android_windowAnimationStyle, 0);
            a.recycle();
        }
        ...
        public MenuView getListMenuView(MenuPresenter.Callback cb) {
            ...
            if (this.listMenuPresenter == null) {
                this.listMenuPresenter = new ListMenuPresenter(this.listPresenterContext, C0191R.layout.abc_list_menu_item_layout);
    ...
    private ViewGroup createSubDecor() {
        ...
        TypedArray a = this.mContext.obtainStyledAttributes(C0191R.styleable.AppCompatTheme);
        if (!a.hasValue(C0191R.styleable.AppCompatTheme_windowActionBar)) {
            a.recycle();
            throw new IllegalStateException("You need to use a Theme.AppCompat theme (or descendant) with this activity.");
        }
        if (a.getBoolean(C0191R.styleable.AppCompatTheme_windowNoTitle, false)) {
            requestWindowFeature(1);
        } else if (a.getBoolean(C0191R.styleable.AppCompatTheme_windowActionBar, false)) {
            requestWindowFeature(108);
        }
        if (a.getBoolean(C0191R.styleable.AppCompatTheme_windowActionBarOverlay, false)) {
            requestWindowFeature(109);
        }
        if (a.getBoolean(C0191R.styleable.AppCompatTheme_windowActionModeOverlay, false)) {
            requestWindowFeature(10);
        }
        this.mIsFloating = a.getBoolean(C0191R.styleable.AppCompatTheme_android_windowIsFloating, false);
        a.recycle();
        ...
        if (this.mWindowNoTitle) {
            if (this.mOverlayActionMode) {
                subDecor = (ViewGroup) inflater.inflate(C0191R.layout.abc_screen_simple_overlay_action_mode, null);
            } else {
                subDecor = (ViewGroup) inflater.inflate(C0191R.layout.abc_screen_simple, null);
            }
            ...
            subDecor = (ViewGroup) inflater.inflate(C0191R.layout.abc_dialog_title_material, null);
            this.mOverlayActionBar = false;
            this.mHasActionBar = false;
        } else if (this.mHasActionBar) {
            TypedValue outValue = new TypedValue();
            this.mContext.getTheme().resolveAttribute(C0191R.attr.actionBarTheme, outValue, true);
            if (outValue.resourceId != 0) {
                themedContext = new ContextThemeWrapper(this.mContext, outValue.resourceId);
            } else {
                themedContext = this.mContext;
            }
            subDecor = (ViewGroup) LayoutInflater.from(themedContext).inflate(C0191R.layout.abc_screen_toolbar, null);
            this.mDecorContentParent = (DecorContentParent) subDecor.findViewById(C0191R.C0193id.decor_content_parent);
        ...
        if (this.mDecorContentParent == null) {
            this.mTitleView = (TextView) subDecor.findViewById(C0191R.C0193id.title);
        }
        ViewUtils.makeOptionalFitsSystemWindows(subDecor);
        ContentFrameLayout contentView = (ContentFrameLayout) subDecor.findViewById(C0191R.C0193id.action_bar_activity_content);
    ...
    private void applyFixedSizeWindow() {
        ...
        TypedArray a = this.mContext.obtainStyledAttributes(C0191R.styleable.AppCompatTheme);
        a.getValue(C0191R.styleable.AppCompatTheme_windowMinWidthMajor, cfl.getMinWidthMajor());
        a.getValue(C0191R.styleable.AppCompatTheme_windowMinWidthMinor, cfl.getMinWidthMinor());
        if (a.hasValue(C0191R.styleable.AppCompatTheme_windowFixedWidthMajor)) {
            a.getValue(C0191R.styleable.AppCompatTheme_windowFixedWidthMajor, cfl.getFixedWidthMajor());
        }
        if (a.hasValue(C0191R.styleable.AppCompatTheme_windowFixedWidthMinor)) {
            a.getValue(C0191R.styleable.AppCompatTheme_windowFixedWidthMinor, cfl.getFixedWidthMinor());
        }
        if (a.hasValue(C0191R.styleable.AppCompatTheme_windowFixedHeightMajor)) {
            a.getValue(C0191R.styleable.AppCompatTheme_windowFixedHeightMajor, cfl.getFixedHeightMajor());
        }
        if (a.hasValue(C0191R.styleable.AppCompatTheme_windowFixedHeightMinor)) {
            a.getValue(C0191R.styleable.AppCompatTheme_windowFixedHeightMinor, cfl.getFixedHeightMinor());
        }
        a.recycle();
        cfl.requestLayout();
    }
        ...
        if (mode != null) {
            ...
            if (this.mActionModeView == null) {
                if (this.mIsFloating) {
                    ...
                    baseTheme.resolveAttribute(C0191R.attr.actionBarTheme, outValue, true);
                    if (outValue.resourceId != 0) {
                    ...
                    this.mActionModePopup = new PopupWindow(actionBarContext, null, C0191R.attr.actionModePopupWindowStyle);
                    PopupWindowCompat.setWindowLayoutType(this.mActionModePopup, 2);
                    this.mActionModePopup.setContentView(this.mActionModeView);
                    this.mActionModePopup.setWidth(-1);
                    actionBarContext.getTheme().resolveAttribute(C0191R.attr.actionBarSize, outValue, true);
                    ...
                    ViewStubCompat stub = (ViewStubCompat) this.mSubDecor.findViewById(C0191R.C0193id.action_mode_bar_stub);
                    if (stub != null) {
    ...
    private void openPanel(PanelFeatureState st, KeyEvent event) {
        if (!st.isOpen && !isDestroyed()) {
            ...
            if (cb == null || cb.onMenuOpened(st.featureId, st.menu)) {
                WindowManager wm = (WindowManager) this.mContext.getSystemService(TiC.PROPERTY_WINDOW);
                if (wm != null && preparePanel(st, event)) {
                    ...
                    st.isHandled = false;
                    WindowManager.LayoutParams lp3 = new WindowManager.LayoutParams(width, -2, st.f10x, st.f11y, 1002, 8519680, -3);
    ...
    private boolean initializePanelMenu(PanelFeatureState st) {
        ...
        if ((st.featureId == 0 || st.featureId == 108) && this.mDecorContentParent != null) {
            ...
            if (outValue.resourceId != 0) {
                ...
                widgetTheme.applyStyle(outValue.resourceId, true);
                widgetTheme.resolveAttribute(C0191R.attr.actionBarWidgetTheme, outValue, true);
            } else {
                baseTheme.resolveAttribute(C0191R.attr.actionBarWidgetTheme, outValue, true);
            }
            if (outValue.resourceId != 0) {
                if (widgetTheme == null) {
                    widgetTheme = context.getResources().newTheme();
            ...
            if (this.mActionModeView.isShown()) {
                ...
                if (mlp.topMargin != newMargin) {
                    ...
                    if (this.mStatusGuard == null) {
                        ...
                        this.mStatusGuard.setBackgroundColor(this.mContext.getResources().getColor(C0191R.color.abc_input_method_navigation_guard));
